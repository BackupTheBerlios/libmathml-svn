...
writeCode() {
    ...
    for (l = at.begin(); l != at.end(); ++l) {
        ...
        if (m != custattlist.end()) {
            ...
            ucatty.push_back(mkvar(m->second));
            ucats.insert(mkvar(m->second));
        } else {
...
//    f << "\tvirtual MMLAttribute *copy() const = 0;" << endl;
    ...
    for (l = atty.begin(); l != atty.end(); ++l) {
        f << "class MML" << *l << " : public MMLAttribute {" << endl;
        f << "public:" << endl;
        f << "\tMML" << *l << "(" << *l << ", bool c=true);" << endl;
        f << "\t" << *l << " value;" << endl;
